...
| OUTPUT_FINISHED | Individual output completed | When each output is ready |
| RUN_FINISHED | Processing complete | When run succeeds or fails |

<Tip>
The `OUTPUT_FINISHED` event is useful for real-time progress tracking and starting post-processing on individual outputs before the entire run completes.
</Tip>
...
For individual output completion (`OUTPUT_FINISHED`):

```json
{
  "flag": "OUTPUT_FINISHED",
  "agent_id": "123e4567-e89b-12d3-a456-789012345678",
  "run_id": "7f8d9c2b-4a6e-8b3f-1d5c-9e2f3a4b5c6d",
  "status": "running",
  "output": {
    "video_url": "https://storage.googleapis.com/mosaic-outputs/output-1.mp4",
    "thumbnail_url": "https://storage.googleapis.com/mosaic-thumbnails/thumb-1.jpg",
...
| --- | --- | --- |
| flag | string | Event type: `RUN_STARTED`, `OUTPUT_FINISHED`, or `RUN_FINISHED` |
...
| output | object | Single output (only present in `OUTPUT_FINISHED` events) |
...
| output.completed_at | string | When this specific output was completed |
| outputs_completed | number | Number of outputs completed so far (`OUTPUT_FINISHED` only) |
| outputs_total | number | Total expected outputs (`OUTPUT_FINISHED` only) |
...
def handle_mosaic_webhook(token):
    ...
    elif flag == 'OUTPUT_FINISHED':
        output = data.get('output', {})
        outputs_completed = data.get('outputs_completed', 0)
        outputs_total = data.get('outputs_total', 0)
        
        logger.info(f"Output {outputs_completed}/{outputs_total} completed for run {run_id}")
        
        # Process the individual output
