...
### Request
```bash
curl -X GET "https://api.usemosaic.ai/videos/get_upload_url" \
...
### Request
```bash
curl -X POST "https://api.usemosaic.ai/videos/finalize_upload" \
  -H "Authorization: Bearer mk_your_api_key" \
...
RESPONSE=$(curl -s -X GET "https://api.usemosaic.ai/videos/get_upload_url" \
