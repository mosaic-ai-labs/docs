...
When a webhook secret is configured for your account, Mosaic includes an `X-Mosaic-Signature` header for verification. If configured, always validate this signature before processing webhook payloads. See the [Webhooks](/overview/webhooks) documentation for details.
