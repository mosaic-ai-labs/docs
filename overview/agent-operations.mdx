...
Execute an agent workflow on one or more uploaded videos or videos from signed URLs.

### Running with Video IDs

Use `video_ids` when you've already uploaded videos to Mosaic:
...
### Running with Signed URLs

You can also run agents directly with signed URLs pointing to external videos. The backend will automatically download the video, upload it to your storage, create a video record, and run the agent workflow.
...
curl -X POST "https://api.mosaic.so/agent/[agent_id]/run" \
  ...
  -H "Content-Type: application/json" \
  -d '{
    "signed_urls": ["https://storage.example.com/video.mp4?signature=..."],
...
| --- | --- | --- | --- |
| video_ids | string[] | ✅* | Array of video IDs to process (for previously uploaded videos) |
| signed_urls | string[] | ✅* | Array of signed URLs (HTTP/HTTPS) pointing to videos to process |
...
\* Either `video_ids` or `signed_urls` must be provided, but not both.

### Signed URL Requirements & Limits

When using `signed_urls`, the following requirements apply:
...
When you provide `signed_urls`, the backend:

1. Validates the signed URL (must be HTTP/HTTPS)
2. Downloads the video from the signed URL
3. Extracts metadata (duration, resolution, etc.)
4. Validates file size (max 5 GB) and duration (max 1 hour)
5. Uploads the video to your storage (GCS/S3)
6. Creates a video record in the database
7. Sets up transcoding
8. Queues embedding task
9. Creates agent state and runs the workflow

The signed URL is automatically converted to a video record, so the rest of the workflow treats it like any other video input.
...
url = "https://api.mosaic.so/agent/{agent_id}/run"
headers = {
