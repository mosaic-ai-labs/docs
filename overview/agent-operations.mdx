...
First, create an agent on [app.usemosaic.ai](https://app.usemosaic.ai). Once created, you can find the agent ID in the URL when viewing your agent.
...
### Request
```bash
curl -X GET "https://api.usemosaic.ai/agent_run/[run_id]" \
  -H "Authorization: Bearer mk_your_api_key"
```
...
response = requests.get(f"{BASE_URL}/videos/get_upload_url", headers=headers)
upload_data = response.json()

# Step 2: Upload video
with open("my-video.mp4", "rb") as video_file:
    requests.put(upload_data["upload_url"], data=video_file, 
