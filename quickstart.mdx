...
```bash
# OPTION A – Let Mosaic choose an agent based on a natural-language prompt
curl -s -X POST "$BASE_URL/run-agent" \
  -H "Authorization: Bearer $MOSAIC_API_KEY" \
  -H "Content-Type: application/json" \
  -d "{\n  \"file_id\": \"$FILE_ID\",\n  \"auto\": true,\n  \"prompt\": \"Create a punchy 30-second trailer with background music and captions\"\n}" > run.json

# OPTION B – Target a specific agent (list yours with GET /get-user-agents-simple)
# curl -s -X POST "$BASE_URL/run-agent" \
#   -H "Authorization: Bearer $MOSAIC_API_KEY" \
#   -H "Content-Type: application/json" \
